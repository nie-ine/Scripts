...
class GenericClass(PCBase):
    def __init__(self, resourceName, propertyList, valueList):
        
        #propertyList bleibt gleich, wholeValueList aendert sich
        
        super(PCBase, self).__init__()
        global _SEQNUM
        
        self._PROJ_PREFIX = _PROJ_PREFIX
        self._RESTYPE = resourceName
        self._RESOURCE_ID = None
        ...
        #print("\nPropertyList in Generic Class: {}".format(propertyList))
        #print("ValueList in Generic Class: {}\n".format(valueList))
        
        if type(propertyList) is list:
            #print("Its a list")
            for i in range(len(propertyList)):
                setattr(self, propertyList[i], valueList[i])
        else:
            #print("Its not a list")
            propertyListHelp = []
            valueListHelp = []
            propertyListHelp.append(propertyList) 
            valueListHelp.append(valueList) 
            setattr(self, propertyListHelp[0], valueListHelp[0])
