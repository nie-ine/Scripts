...
class Knora(object):
    ...
    def __init__(self, server, auth=None):
        ...
        self.put_url = "{}{}".format(server, _PUT_PATH)
    ...
    ## zieht alle informationen bezueglich einer ResourceID aus Knora
    def get_resource(self, resource_id=None):
        if resource_id:
            try:
                r = self.session.get("{}/{}".format(self.post_url, resource_id))
                r.raise_for_status()
                return r.json()
            except Exception:
    ...
    ## update einer Resource Property
    def update_resource_property(self, resource_id, prop_name, value):
        # !!! propname MUSS zur Zeit noch mit 'Ontologienamen' als Prefix uebergeben werden
        #     z.B. nie-ine:name, dc:name, etc.
        
        if resource_id and prop_name:
            resource_info = self.get_resource(resource_id)
            #print(resource_info)
            try:
                property_id = resource_info['props'][prop_name]['value_ids'][0]
                if property_id:
                    url = "{}/{}".format(self.put_url, property_id)
                    data = {"value": value}
